...
class Logger {
 public:
  Logger(int& argsize, char**& program, std::string&& path);
  ~Logger();
};  // namespace Logger
...
#define ALOG_MODULE_STREAM_INFO(module) google::LogMessage(__FILE__, __LINE__, google::INFO).stream() << LEFT_BRACKET << module << RIGHT_BRACKET

#define ALOG_MODULE_STREAM_WARN(module) google::LogMessage(__FILE__, __LINE__, google::WARNING).stream() << LEFT_BRACKET << module << RIGHT_BRACKET

#define ALOG_MODULE_STREAM_ERROR(module) google::LogMessage(__FILE__, __LINE__, google::ERROR).stream() << LEFT_BRACKET << module << RIGHT_BRACKET

#define ALOG_MODULE_STREAM_FATAL(module) google::LogMessage(__FILE__, __LINE__, google::FATAL).stream() << LEFT_BRACKET << module << RIGHT_BRACKET
